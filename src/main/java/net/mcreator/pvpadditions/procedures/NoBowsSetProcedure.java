...
public class NoBowsSetProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, CommandContext<CommandSourceStack> arguments) {
		if (BoolArgumentType.getBool(arguments, "TrueOrFalse") == true) {
			world.getLevelData().getGameRules().getRule(PvpAdditionsModGameRules.ENABLE_BOWS).set(true, world.getServer());
			...
			world.getLevelData().getGameRules().getRule(PvpAdditionsModGameRules.ENABLE_BOWS).set(false, world.getServer());
			if (world instanceof ServerLevel _level)
				_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
						"/tellraw @p [\"\",{\"text\":\"PvP\",\"bold\":true,\"color\":\"dark_blue\"},{\"text\":\"ADDITIONS >> \",\"bold\":true,\"color\":\"blue\"},{\"text\":\"Action Successful\",\"color\":\"green\"}]");
		}
